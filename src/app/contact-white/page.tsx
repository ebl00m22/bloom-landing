...
export default function ContactWhitePage() {
  return (
    <main className="overflow-x-hidden bg-white">

      {/* ── HERO ── */}
      <section className="relative overflow-hidden min-h-screen flex items-center bg-[#f8faf9]">

        {/* Orbs — very faint on light */}
        <div className="absolute rounded-full blur-[180px] pointer-events-none" style={{ width: 700, height: 700, top: "-15%", right: "-8%", background: "#004845", opacity: 0.07 }} />
        <div className="absolute rounded-full blur-[160px] pointer-events-none" style={{ width: 500, height: 500, bottom: "-10%", left: "-5%", background: "#e17339", opacity: 0.06 }} />

        {/* Ghost B mark — multiply blend on light */}
        <div
          aria-hidden="true"
          className="absolute right-[-8%] top-1/2 -translate-y-1/2 w-[55vw] max-w-[700px] pointer-events-none select-none"
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
            ...
            <div>
              <motion.p
                ...
                className="text-sm font-semibold tracking-widest uppercase text-[#e17339] mb-4"
              >
                Contact Us
              </motion.p>
              <motion.h1
              ...
              >
                {callExpectations.map((item, i) => (
                  <motion.div key={i} variants={fadeUp} className="flex gap-4">
                    <div className="w-10 h-10 bg-[#0c0a14]/[0.05] border border-[#0c0a14]/[0.08] rounded-xl flex items-center justify-center text-[#e17339] shrink-0">
                      {item.icon}
                    </div>
                    <div>
      ...
      <section className="bg-white py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          >
            <p className="text-sm font-semibold tracking-widest uppercase text-[#e17339] mb-3">Why Bloom Social</p>
            <h2 className="text-3xl md:text-4xl font-extrabold text-[#004845] leading-tight">You&apos;re in Good Hands</h2>
          </motion.div>
          <motion.div
      ...
      <section className="bg-[#f7f8f7] py-16 md:py-20">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          <motion.div
            ...
            className="text-center mb-12"
          >
            <h2 className="text-3xl font-extrabold text-[#004845]">Before You Reach Out</h2>
          </motion.div>
          <motion.div
          ...
          >
            {[
              { q: "How quickly will you respond?",     a: "Within 24 hours. Usually the same business day." },
              { q: "Is the strategy session really free?", a: "Yes, no strings attached. You keep the plan even if we don't work together." },
              { q: "Do you only work with executives?", a: "We specialize in executive social media, but we work with businesses of all types and sizes." },
              { q: "Do I need to be in Michigan?",      a: "Not at all. We serve clients across the entire country." },
            ].map((faq, i) => (
      ...
      {/* ── BOTTOM CTA ── */}
      <section className="bg-[#004845] py-14 md:py-16">
        <div className="max-w-5xl mx-auto px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <p className="text-white font-extrabold text-2xl md:text-3xl leading-snug">Not ready for a call yet?</p>
            <p className="text-white/60 mt-1">Check out our services or read about our process first.</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="flex flex-col sm:flex-row gap-3 shrink-0">
            <Link href="/services" className="inline-flex items-center gap-2 px-7 py-3.5 bg-white text-[#004845] font-bold rounded-full text-sm hover:bg-white/90 transition-colors duration-200">
              Our Services <ArrowRightIcon className="w-4 h-4" />
            </Link>
            <Link href="/process" className="inline-flex items-center gap-2 px-7 py-3.5 bg-white/[0.10] border border-white/20 hover:bg-white/[0.18] text-white font-semibold rounded-full text-sm transition-all duration-200">
              How We Work
            </Link>
          </motion.div>
        </div>
